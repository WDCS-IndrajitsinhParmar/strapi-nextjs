...
const ProductImages = (props: ProductImagesProps) => {
    const { values } = props;
    const [imgList, setImgList] = useState<Image[]>(values.imgList || []);
    ...
    const onUpload = (files: File[]) => {
        ...
        const newImgList = [...imgList, image];
        setImgList(newImgList);
    };

    const handleImageDelete = (deletedImg: Image) => {
        const newImgList = imgList.filter((img) => img.id!== deletedImg.id);
        setImgList(newImgList);
    };
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                    {imgList.map((img) => (
                        <div key={img.id} className="group relative rounded border p-2 flex">
                            ...
                            <div className="absolute inset-2 bg-gray-900/[.7] group-hover:flex hidden text-xl items-center justify-center">
                                <span className="text-gray-100 hover:text-gray-300 cursor-pointer p-1.5" onClick={() => {}}>
                                    <HiEye />
                                </span>
                                <span className="text-gray-100 hover:text-gray-300 cursor-pointer p-1.5" onClick={() => {}}>
                                    <HiTrash />
